...
Default seed (always created): S3 bucket `emulate-default`, DynamoDB table `emulate-default`, SQS queue `emulate-default-queue`, IAM user `admin` with access key pair.
