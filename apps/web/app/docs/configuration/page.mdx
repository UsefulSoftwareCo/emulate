...
Seed data is replayed by `reset()` in the programmatic API and by `POST /_emulate/reset` on a running emulator instance. The request ledger at `/_emulate/ledger` is also cleared during reset.
...
Use the standalone MCP emulator's `oauth` seed block to exercise RFC 7591, RFC 8414, and RFC 9728 client failure handling. The routes read this configuration for each request, so posting a new seed to `/_emulate/seed` updates a running instance:
...
`tokenEndpointAuthMethods` may be an exact advertised list or the string `omit`. `dcrAuthMethodOverride` replaces the requested registration method in both stored client metadata and the DCR response. Confidential clients registered as `client_secret_basic` must authenticate at `/token` with HTTP Basic, while `client_secret_post` clients must use the form body.
...
Microsoft `drive_items[].content` is a plain UTF-8 seed string. Runtime file uploads use Graph content endpoints and are stored internally as base64 for byte-exact binary downloads.
