...
All 13 services are available on host-based routing. There are four address forms:
...
Instance names are arbitrary, so a CI run can pick a unique one such as `stripe.ci-48291.emulators.dev`. The instance host form is preferred for public examples because the provider base URL is the origin, which matches a service-owned host.
...
Instances are created lazily. Creating one, or simply addressing an instance host for the first time, returns the base URL, control base URL, and provider base URL for that instance. See the [Service Manifest](/docs/manifest) for the instance shape and [Request Ledger](/docs/ledger) for what persists.
